...
class RegressionMetric(CometModel):
    ...
    def forward(
        ...
        **kwargs
        ...
        # Mekael Added for precision 16
        # Ensure input_ids are of type torch.long
        # src_input_ids = src_input_ids.long()
        # mt_input_ids = mt_input_ids.long()
        # ref_input_ids = ref_input_ids.long()
        
        # Ensure attention_mask is of type torch.float32
        src_attention_mask = src_attention_mask.float()
        mt_attention_mask = mt_attention_mask.float()
        ref_attention_mask = ref_attention_mask.float()
